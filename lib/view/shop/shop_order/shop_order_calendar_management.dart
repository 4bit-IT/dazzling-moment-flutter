...
class ShopOrderCalendarManagement extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(
            ...
            eventLoader: (day) =>
            ownerOrderDayController.event[day] ?? [],
              ...
              markerBuilder: (context, dateTime, List event) {
                ...
                return Obx(() {
                  if (ownerOrderDayController.event[dateTime] != null) {
                    //디폴트 값 아님
                    return ownerOrderDayController
                    ...
                    return ownerOrderDayController
                        .dayTime[dayOfWeek][0].isHoliday!
                        ? Text(
                      '휴일',
                      style: TextStyle(color: Colors.lightBlue),
                    )
                        : Text(
                        '${ownerOrderDayController.dayTime[dayOfWeek][0].startTime} - ${ownerOrderDayController.dayTime[dayOfWeek][0].endTime}');
                  }
                });
              },
            ),
  ...
  void changeDateTime() {
    if (!ownerOrderDayController.checkAvailableTime(
        ...
        ownerOrderDayController.selectedHoliday.value)) {
      GetDialog().simpleDialog('종료시간을 시간시간 이후로 설정해주세요. 단, 종료시간만 0시 인 경우 제외');
      return;
    }
    if (ownerOrderDayController
        .event[ownerOrderDayController.focusedDay.value] ==
        null) {
      //원래 디폴트 값이엇으면,
